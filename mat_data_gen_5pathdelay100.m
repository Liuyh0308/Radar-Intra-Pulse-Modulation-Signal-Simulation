...
datasetCWD = 'D:\radar data\dataset-MAT-5pathdelay100';
...
atenuations = [0, -1.5, -1.4, -3.6, -9.1];% path gains
% delays = [0,delayset(randperm(length(delayset),5))]; % path delays
delays = [0, 30, 150, 310, 710]*1e-9;% path delays
...
%                     multipathChannel.MaximumDopplerShift = randi([10 1000]);
                    multipathChannel.MaximumDopplerShift = 100;
...
%                     wav = awgn(wav,SNR,'measured');
                    filename = fullfile(waveformfolderCWD,sprintf('rect-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
...
%                     multipathChannel.MaximumDopplerShift = randi([10 1000]);
                    ...
                    wav = awgn(wav,SNR(n),'measured');
%                     wav = awgn(wav,SNR,'measured');
                    filename = fullfile(waveformfolderCWD,sprintf('LFM-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('Costas-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('Barker-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('Frank-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('P1-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('P2-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('P3-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,2);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('P4-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,10);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('T1-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     wav = awgn(wav,SNR,'measured');
                    wav = repelem(wav,10);
                    wav = wav(1:1024);
                    filename = fullfile(waveformfolderCWD,sprintf('T2-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     multipathChannel.MaximumDopplerShift = randi([10 1000]);
                    ...
                    wav = awgn(wav,SNR(n),'measured');
%                     wav = awgn(wav,SNR,'measured');
                    filename = fullfile(waveformfolderCWD,sprintf('T3-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
...
%                     multipathChannel.MaximumDopplerShift = randi([10 1000]);
                    ...
                    wav = awgn(wav,SNR(n),'measured');
%                     wav = awgn(wav,SNR,'measured');
                    filename = fullfile(waveformfolderCWD,sprintf('T4-snr%02d-no%05d.mat',SNR(n),idx));
                    label = (waveforms(k));
                    save(filename,"wav","label");
%                     save(filename,"wav");
